...
let updateOpponentScreen = function(opponentGuess){
    ...
    for( let i = OPPONENT_SCREEN_START; i <= OPPONENT_SCREEN_END; i++){
      if(opponentGuess[n] === '1'){
        get_element_function(i).style.animation =  'mymove 1s 1 linear forwards ' + String(1.5*(n + 1)) + 's'
      }
      else if (opponentGuess[n] === '2'){
        get_element_function(i).style.animation =  'mymove2 1s 1 linear forwards ' + String(1.5*(n + 1)) + 's'
      }
      else{
        get_element_function(i).style.animation =  'mymove3 1s 1 linear forwards ' + String(1.5*(n + 1)) + 's'
      }
